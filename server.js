...
app.post('/send-otp', (req, res) => {
    ...
    // Create transporter: prefer explicit SMTP when MAIL_HOST present
    ...
    if (process.env.MAIL_HOST) {
        transporterConfig = {
            ...
            secure: Number(process.env.MAIL_PORT) === 465, // true for 465, false for 587/2525
            tls: { rejectUnauthorized: false }
        };
        console.log('MAIL_USER=' + (process.env.MAIL_USER || ''));
        console.log('MAIL_HOST=' + process.env.MAIL_HOST + ':' + (process.env.MAIL_PORT || ''));
    ...
    const mailOptions = {
        from: process.env.MAIL_FROM || process.env.EMAIL_USER || 'no-reply@letscode.test',
        to: email,
...
app.post('/forgot-password', (req, res) => {
    ...
    const resetLink = `http://localhost:3888/reset-password?email=${encodeURIComponent(email)}`;
        ...
        // Use Mailtrap SMTP when configured, otherwise fall back to Gmail
        let transporterConfig2;
        if (process.env.MAIL_HOST) {
            transporterConfig2 = {
                host: process.env.MAIL_HOST,
                port: Number(process.env.MAIL_PORT) || 587,
            ...
            console.log('Using custom SMTP transport for reset email:', process.env.MAIL_HOST + ':' + process.env.MAIL_PORT);
        } else {
            transporterConfig2 = {
                service: 'gmail',
                auth: {
            ...
            console.log('Using Gmail transport for reset email');
        }

        const transporter = nodemailer.createTransport(transporterConfig2);
...
// Start server with EADDRINUSE handling and automatic port fallback
function startServer(initialPort, attempt = 0) {
    const port = Number(initialPort) || 3888;
    ...
    server.on('error', (err) => {
        if (err && err.code === 'EADDRINUSE') {
            ...
            if (attempt < 5) {
                ...
                console.log(`Trying next port ${nextPort} (attempt ${attempt + 1})...`);
...
// Debug endpoint - shows exact transporter config (with masked password)
app.get('/debug-mail', (req, res) => {
    const config = {
        ...
        MAIL_PASS: process.env.MAIL_PASS ? (process.env.MAIL_PASS.substring(0, 3) + '***' + process.env.MAIL_PASS.substring(process.env.MAIL_PASS.length - 3)) : 'NOT SET',
        MAIL_FROM: process.env.MAIL_FROM || 'NOT SET'
    };
    res.json({ config, message: 'Mailtrap config loaded from .env' });
});

// Mail test endpoint - verifies transporter and tries sending a small test email
app.get('/mail-test', async (req, res) => {
    const to = req.query.to;
    if (!to) return res.status(400).json({ message: 'Provide ?to=deepakchowdar1991@gmail.com' });
    ...
    try {
        ...
        return res.status(500).json({ message: 'Transport verify failed', error: err.toString() });
    }

    const mailOptions = {
        from: process.env.MAIL_FROM || process.env.EMAIL_USER || 'no-reply@letscode.test',
        to,
        subject: 'Test email from LetsCode',
        text: 'This is a test email sent from the LetsCode backend to verify SMTP.'
    };

    transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
            console.error('Test email send failed:', error);
            return res.status(500).json({ message: 'Test email send failed', error: error.toString() });
        }
